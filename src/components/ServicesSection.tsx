...
const services = [
  { icon: Grid3X3, headline: "Exclusive Paver Designs. Not the Pattern Everyone Else Lays.", body: "Noel installs geo mesh beneath every paver project — a weed-blocking layer most crews skip. The result holds longer and keeps looking clean years after installation.", image: paversImg },
  { icon: HardHat, headline: "Functional Concrete Done with Craftsmanship.", body: "Driveways, slabs, RV pads, sidewalks, steps. Properly graded for runoff. Expansion joints placed right. Straight lines and a clean finish.", image: standardImg },
  { icon: Fence, headline: "Stay With One Contractor.", body: "Redoing the backyard? Noel does fencing too. Most clients find this out mid-project and save themselves from coordinating a second contractor." },
  { icon: Blocks, headline: "Retaining Walls and Block Work Built to Last.", body: "Retaining walls, block walls, brick, and stone features. Structural hardscape that holds up to Willamette Valley weather and soil conditions." },
  { icon: Droplets, headline: "Where the Water Goes Matters.", body: "Gutter installation, drain lines, and stormwater drainage planned alongside every driveway or patio project." },
];
    ...
    <section id="services" className="py-20 md:py-28 bg-background">
      <div className="container mx-auto px-4 lg:px-8">
        <div ref={header.ref} className={`text-center max-w-2xl mx-auto mb-16 scroll-hidden ${header.isVisible ? "scroll-visible" : ""}`}>
          ...
          <p className="text-lg text-muted-foreground">Most clients come to us for concrete and leave wondering why they didn't call sooner about everything else.</p>
        </div>

        <div ref={featured.ref} className={`rounded-xl overflow-hidden border-2 border-primary/20 mb-12 scroll-hidden ${featured.isVisible ? "scroll-visible" : ""}`}>
        ...
        <div ref={grid.ref} className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {services.map((s, i) => (
            <div key={s.headline} className={`rounded-xl border border-border bg-card overflow-hidden hover:border-primary/30 transition-colors group scroll-hidden delay-${Math.min(i + 1, 5)} ${grid.isVisible ? "scroll-visible" : ""}`}>
