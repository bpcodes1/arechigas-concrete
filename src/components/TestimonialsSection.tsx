      ...
      <div className="container mx-auto px-4 lg:px-8">
        ...
        <div ref={grid.ref} className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {testimonials.map((t, i) => (
            <div key={t.name} className={`rounded-xl border border-border bg-card p-6 flex flex-col scroll-hidden delay-${Math.min(i + 1, 5)} ${grid.isVisible ? "scroll-visible" : ""}`}>
              <Stars />
        ...
        <p className={`text-center mt-12 text-lg font-semibold text-foreground scroll-hidden delay-1 ${grid.isVisible ? "scroll-visible" : ""}`}>
          Noel has never left a job at less than 5 stars. <span className="text-primary">86 homeowners and counting.</span>
        </p>

        <div className={`text-center mt-6 scroll-hidden delay-2 ${grid.isVisible ? "scroll-visible" : ""}`}>
          <a href="#contact" className="cta-lime px-8 py-4 rounded-md text-lg font-bold transition-all hover:scale-105 inline-block">See What Your Project Costs — Free Quote</a>
        </div>
      </div>
    </section>
  );
